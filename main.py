...
def GetUserInfo(userid):
    return go.CallApi('get_stranger_info', {'user_id':userid}, uuid=go.yamldata.get('self').get('defaultUuid')).get('data')

def load_image(path: str):
    return BuildImage.open("/pbf/resources/images/" + path).convert("RGBA")

def make_jpg_or_gif(meta_data, img, func, gif_zoom=1, gif_max_frames=50) -> BytesIO:
    """
    制作静图或者动图
    :params
      * ``img``: 输入图片，如头像
    ...
    if not getattr(image, "is_animated", False):
        ...
        return save_gif(meta_data, frames, duration)

def save_gif(meta_data, frames: List[IMG], duration: float):
    output = '/resources/createimg/{0}.gif'.format(time.time())
    imageio.mimsave('/pbf'+output, frames, format="gif", duration=duration)
    go.send(meta_data, '[CQ:image,file=file:///www/gch/python{0}]'.format(output))
    return output
    
def GetImage(meta_data):
...
def petpet(meta_data):
    ...
    save_gif(meta_data, frames, 0.06)
...
def littleangel(meta_data):
    userid = meta_data.get('message')
    if 'at' in userid:
        userid = go.getCQValue('qq', userid)
    
    username = GetUserInfo(userid).get('nickname')
    img = GetImage(meta_data).convert("RGBA").resize_width(500)
...
def coupon(meta_data):
    ...
    img = GetImage(meta_data).convert("RGBA").circle().resize((60, 60)).rotate(22, expand=True)
...
def listen_music(meta_data):
    ...
    save_gif(meta_data, frames, 0.05)
    
def throw_gif(meta_data):
    img = GetImage(meta_data).convert("RGBA").circle()
    locs = [
        [(32, 32, 108, 36)],
        [(32, 32, 122, 36)],
        [],
    ...
    for i in range(8):
        ...
        for w, h, x, y in locs[i]:
            frame.paste(img.resize((w, h)), (x, y), alpha=True)
        frames.append(frame.image)
    save_gif(meta_data, frames, 0.1)
    
def eat(meta_data):
    img = GetImage(meta_data).convert("RGBA").square().resize((32, 32))
    frames = []
    for i in range(3):
...
def my_friend(meta_data):
    userid = meta_data.get('args')[1]
    if 'at' in userid:
        userid = go.getCQValue('qq', userid)
    
    name = GetUserInfo(userid).get('nickname')
    texts = meta_data.get('args')[2]
...
def kiss(meta_data):
    ...
    user_head = GetImage(meta_data).convert("RGBA").circle().resize((50, 50))
    # fmt: off
    user_locs = [
...
def pat(meta_data):
    ...
    return save_gif(meta_data, frames, 0.085)

def rip(meta_data):
    img = GetImage(meta_data).convert("RGBA").square().resize((385, 385))
...
def always(meta_data):
    ...
    return make_jpg_or_gif(meta_data, GetImage(meta_data), make)

def loading(meta_data):
    img = GetImage(meta_data)
    ...
    return make_jpg_or_gif(meta_data, img, make)
...
def turn(meta_data):
    ...
    if random.randint(0, 1):
        frames.reverse()
    return save_gif(meta_data, frames, 0.05)
    
def dont_touch(meta_data):
...
def worship(meta_data):
    ...
    for i in range(10):
        ...
        frames.append(frame.image)
    return save_gif(meta_data, frames, 0.04)

def universal(meta_data):
    args = meta_data.get('args')[2].split('|')
    img = GetImage(meta_data)
...
def bite(meta_data):
    ...
    for i in range(6, 16):
        frame = load_image(f"bite/{i}.png")
        frames.append(frame.image)
    return save_gif(meta_data, frames, 0.07)

def police(meta_data):
...
def twist(meta_data):
    img = GetImage(meta_data).convert("RGBA").square().resize((78, 78))
    # fmt: off
    locs = [
    ...
    for i in range(5):
        ...
        frame.paste(img.rotate(a), (x, y), below=True)
        frames.append(frame.image)
    return save_gif(meta_data, frames, 0.1)

def wallpaper(meta_data):
    ...
    return make_jpg_or_gif(meta_data, GetImage(meta_data), make, gif_zoom=0.5)
...
def shock(meta_data):
    ...
    save_gif(meta_data, frames, 0.01)

def dianzhongdian(meta_data):
    arg = meta_data.get('args')[2]
    if not arg:
        return go.send(meta_data, REQUIRE_ARG)

    trans = go.translator(arg)
    img = GetImage(meta_data).convert("L").resize_width(500)
...
def love_you(meta_data):
    ...
    for i in range(2):
        ...
        frames.append(frame.image)
    save_gif(meta_data, frames, 0.2)

def symmetric(meta_data):
    arg = meta_data.get('args')[2]
    if arg not in ["上", "下", "左", "右"]:
        return go.send(meta_data, '可选方向：上、下、左、右！')
    img = GetImage(meta_data).convert("RGBA").resize_width(500)
    img_w, img_h = img.size
...
def punch(meta_data):
    ...
    for i in range(13):
        ...
        frames.append(frame.image)
    save_gif(meta_data, frames, 0.03)

def cyan(meta_data):
    color = (78, 114, 184)
    frame = GetImage(meta_data).convert("RGB").square().resize((500, 500)).color_mask(color)
    frame.draw_text(
        (400, 40, 480, 280),
        "群\n青",
        max_fontsize=80,
...
def thump(meta_data):
    ...
    for i in range(4):
        ...
        frame.paste(img.resize((w, h)), (x, y), below=True)
        frames.append(frame.image)
    save_gif(meta_data, frames, 0.04)
...
def garbage(meta_data):
    img = GetImage(meta_data).convert("RGBA").square().resize((79, 79))
    # fmt: off
    locs = (
...
def jiujiu(meta_data):
    img = GetImage(meta_data).convert("RGBA").resize((75, 51), keep_ratio=True)
    frames: List[IMG] = []
    for i in range(8):
...
def suck(meta_data):
    ...
    for i in range(12):
        ...
        frames.append(frame.image)
    save_gif(meta_data, frames, 0.08)


def hammer(meta_data):
    img = GetImage(meta_data).convert("RGBA").square()
    # fmt: off
    ...
    for i in range(7):
        ...
        frame.paste(img.resize((w, h)), (x, y), below=True)
        frames.append(frame.image)
    save_gif(meta_data, frames, 0.07)


def tightly(meta_data):
    img = GetImage(meta_data).convert("RGBA").resize((640, 400), keep_ratio=True)
    # fmt: off
...
def keepaway(meta_data):
    imgs = [GetImage(meta_data)]
...
def repeat(meta_data):
    ...
    self_img = GetImage({'message':str(meta_data.get('se').get('user_id'))}).convert("RGBA").circle().resize((75, 75))
